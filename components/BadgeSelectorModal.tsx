...
import { buildDuplicateMessage, findBadgeDuplicates } from '@/src/utils/duplicates';
...
import { Dimensions, KeyboardAvoidingView, LayoutAnimation, Modal, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export function BadgeSelectorModal({ visible, onClose, onSave, initialSelectedIds }: BadgeSelectorModalProps) {
    ...
    const handleCreateBadge = async () => {
        ...
        try {
            const duplicates = findBadgeDuplicates(
                { id: editingBadge?.id, name: newName, group_name: newGroup },
                allBadges,
                3
            );
            ...
            if (duplicates.length > 0) {
                confirm.custom({
                    ...
                    message: buildDuplicateMessage('Ya existe un badge muy similar. ¿Querés guardarlo igual?', duplicates.map((d) => ({ title: d.name, subtitle: d.group_name ?? undefined }))),
                    variant: 'warning',
                    buttons: [
                        { label: 'Cancelar', onPress: confirm.hide, variant: 'ghost' },
                        { label: 'Guardar igualmente', onPress: async () => { confirm.hide(); await doSaveBadge(); }, variant: 'solid' },
                    ]
                });
                return;
            }
    ...
    const doSaveBadge = async () => {
        try {
            ...
            notify.error('Error', (e as Error)?.message || 'No se pudo guardar el badge.');
        }
    };
    ...
    const handleDeleteBadge = async (id: string) => {
        ...
        try {
            ...
            notify.error('Error', (e as Error)?.message || 'No se pudo eliminar el badge.');
        }
    };
    ...
    const filteredBadges = allBadges.filter((b) => matchesKeywordAndQuery(search, [b.name, b.group_name]));
