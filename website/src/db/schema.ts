...
export const themePacks = pgTable('theme_packs', {
    ...
    visibility: text('visibility').$type<'private' | 'friends' | 'public'>().notNull().default('private'),
    status: text('status').$type<'draft' | 'pending_review' | 'approved' | 'rejected' | 'suspended'>().notNull().default('draft'),
...
export const themePackFeedback = pgTable('theme_pack_feedback', {
    ...
    themePackId: text('theme_pack_id').notNull().references(() => themePacks.id, { onDelete: 'cascade' }),
    userId: text('user_id').notNull(),
    kind: text('kind').$type<'issue' | 'suggestion' | 'praise'>().notNull(),
    message: text('message').notNull(),
...
export const themePackReports = pgTable('theme_pack_reports', {
    ...
    reason: text('reason').$type<'nsfw' | 'hate' | 'spam' | 'impersonation' | 'malware' | 'other'>().notNull(),
